...
import com.vanillapings.api.PingCreatedEvent;
import com.vanillapings.api.PingEvents;
import com.vanillapings.api.PingKind;
...
public class PingManager {
    ...
    public static void pingAtPosition(Vec3 pos, @Nullable Entity pingEntity, Player player, Level world) {
        ...
        int maxAgeTicks = VanillaPings.SETTINGS.getPingDurationTicks();
        PingedEntity pingedEntity = new PingedEntity(targetEntity, maxAgeTicks, animate, kill, VanillaPings.SETTINGS.isPlaySound(), highlight);
        entities.add(pingedEntity);
        pingedEntity.tick();

        PingEvents.fire(new PingCreatedEvent(
                world,
                pos,
                pingEntity != null ? pingEntity.getUUID() : null,
                player.getUUID(),
                player.getName().getString(),
                maxAgeTicks,
                pingEntity != null ? PingKind.ENTITY : PingKind.LOCATION
        ));
    }
